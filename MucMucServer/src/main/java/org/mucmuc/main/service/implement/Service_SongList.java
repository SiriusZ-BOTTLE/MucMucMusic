...
public class Service_SongList implements Interface_SongList_service {
    ...
    public ResultEntity create(SongList songList) {
        ...
        try
        {
            ...
            {
                resultEntity.setInfo_error("<ERROR> 歌单名为空或歌单名为空串");
                return resultEntity;
            }

            //创建
            songListDao.insertNew(songList);
            resultEntity.setState(true);
            return resultEntity;
        }
        catch(Exception e)
        {
            ...
            resultEntity.setInfo_error("创建歌单失败; 捕获到异常");
            return resultEntity;
        }
    }
    ...
    public ResultEntity getUserSongList(User user) {
        ...
        catch(Exception e)
        {
            e.printStackTrace();
            resultEntity.setState(false);
            resultEntity.setInfo_error("; 捕获到异常");
            return resultEntity;
        }
    }
    ...
    public ResultEntity addSongToSongList(Song song, SongList songList) {
        ...
        catch(Exception e)
        {
            e.printStackTrace();
            resultEntity.setState(false);
            resultEntity.setInfo_error("添加歌曲至歌单失败; 捕获到异常");
            return resultEntity;
        }
    }
    ...
    public ResultEntity removeSongFromSongList(Song song, SongList songList) {
        ...
        catch(Exception e)
        {
            e.printStackTrace();
            resultEntity.setState(false);
            resultEntity.setInfo_error("从歌单移除歌曲失败; 捕获到异常");
            return resultEntity;
        }
    }
    ...
    public ResultEntity update(SongList songList) {
        ...
        catch(Exception e)
        {
            e.printStackTrace();
            resultEntity.setState(false);
            resultEntity.setInfo_error("更新歌单信息失败; 捕获到异常");
            return resultEntity;
        }
    }
    ...
    public ResultEntity delete(SongList songList) {
        ...
        catch(Exception e)
        {
            e.printStackTrace();
            resultEntity.setState(false);
            resultEntity.setInfo_error("删除歌单失败; 捕获到异常");
            return resultEntity;
        }
    }
}
